/**
 * SchemaOpenDialog - スキーマ設定開くダイアログ
 *
 * 保存済みスキーマ設定一覧から選択して開く、または削除する
 */
...
export interface SchemaOpenDialogProps {
  /** ダイアログの表示状態 */
  isOpen: boolean;
  /** 保存済みスキーマ設定一覧 */
  savedSettings: SavedSchemaSettings[];
  /** 選択時のコールバック */
  onSelect: (saved: SavedSchemaSettings) => void;
  /** 削除時のコールバック */
...
export function SchemaOpenDialog({
  isOpen,
  savedSettings,
  onSelect,
  onDelete,
  onCancel,
}: SchemaOpenDialogProps) {
  const [deleteTarget, setDeleteTarget] = useState<SavedSchemaSettings | null>(null);
  ...
  const handleDeleteClick = (e: React.MouseEvent, saved: SavedSchemaSettings) => {
    e.stopPropagation();
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
        ...
        >
          ...
          <div className="px-4 py-3 border-b border-stone-700 flex items-center justify-between">
            <h2 className="text-sm font-semibold text-stone-100">保存済みスキーマ設定を開く</h2>
            <button
              type="button"
              onClick={onCancel}
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-stone-400 truncate">
                        レイアウト: {layoutLabel(saved.settings.layoutType)}
                      </div>
