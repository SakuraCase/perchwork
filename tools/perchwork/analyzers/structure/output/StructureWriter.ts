...
export class StructureWriter {
  ...
  async write(fileAnalyses: FileAnalysis[], resolvedEdges: CallEdge[]): Promise<void> {
    ...
    const testedBy = buildTestedByMap(resolvedEdges);
    ...
    for (const analysis of fileAnalyses) {
      ...
      const items = analysis.items.map((item) => {
        const testIds = testedBy.get(item.id);
        return testIds && testIds.length > 0
          ? { ...item, tested_by: testIds }
          : item;
      });
    ...
    console.log(`  - エッジ数: ${resolvedEdges.length}`);
  }
}

function buildTestedByMap(edges: CallEdge[]): Map<string, string[]> {
  const testedBy = new Map<string, string[]>();

  for (const edge of edges) {
    if (!edge.from.endsWith('::test') || edge.to.endsWith('::test')) {
      continue;
    }

    const current = testedBy.get(edge.to) ?? [];
    if (!current.includes(edge.from)) {
      current.push(edge.from);
    }
    testedBy.set(edge.to, current);
  }
