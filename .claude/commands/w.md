...
コードベースの**事実**を記録した wiki 的な知識ベース。

- **内容**: コードの構造、処理フロー、アーキテクチャなど**実装の事実のみ**
- **含めないもの**: リファクタリング案、改善提案、意見、評価、課題指摘
...
| Framer Motion  | アニメーション（限定利用） | 「アニメーション利用ガイドライン」参照        |
...
- prompt: 以下のルールで調査レポートを生成
  ...
  - **表現の質を高める**:
    ...
    - 「UX 原則」に従い、インタラクティブ要素を活用
    - テキストだけで説明可能でも、視覚化で理解が深まる場合はパッケージを活用
    - アニメーションは「アニメーション利用ガイドライン」に従い限定的に使用
  - **スタイル**: 「利用可能なパッケージ」「デザインルール」に従い実装

## サブエージェント 2: ドキュメント更新

コードベースの**事実**を記録した「wiki 的な知識ベース」を作成・更新する。
古いセッションの内容更新はしない。
...
- prompt: 以下のルールで知識ベースを更新
  - **目的**: コードベースの構造・処理フロー・アーキテクチャを事実として記録
  - **記載する内容**:
    - コードの構造（ファイル構成、モジュール関係）
    - 処理フロー（関数の呼び出し順序、データの流れ）
    - アーキテクチャ（レイヤー構成、依存関係）
    - 主要な型・インターフェースの説明
  - **記載しない内容（重要）**:
    - リファクタリング案・改善提案
...
| 処理フロー・手順 | フローチャート         | Mermaid            |
| コード解説       | シンタックスハイライト | PrismJS            |
| 数値比較・統計   | チャート               | ECharts            |

**選択基準**:

- テキストで十分説明できる場合は、テキスト・テーブルを優先
- 視覚化が理解を助ける場合のみパッケージを活用
...
**原則: アニメーションは最小限に。静的な表現で十分な場合は使わない。**

### 使用してはいけない場面

- **装飾目的**: 見た目を良くするためだけのアニメーション
- **ホバーエフェクト**: CSS の `transition` で十分（Framer Motion 不要）
- **単純なフローチャート**: Mermaid で静的に表現可能
- **リストやカードの表示**: 即座に表示すれば良い
- **展開/折りたたみ**: CSS の `transition` で十分

### 使用を検討してよい場面（それでも静的表現を優先）

- **時間経過を伴う処理の可視化**: 複数ステップが順番に実行される様子を示す必要がある場合のみ
- **状態遷移のデモ**: ユーザーが「再生」ボタンで明示的に開始する形式

### 使用する場合の制約

- 必ず「静的表現で代替可能か」を検討してから使用
- 自動再生は避け、ユーザーが明示的に開始できるようにする
- 1ページに複数のアニメーションを配置しない
...
- **ホバー**: 用語や概念の補足説明を表示（CSS transition で実装）
- **クリック**: 詳細セクションの展開/折りたたみ（CSS transition で実装）
- **ツールチップ**: 専門用語や略語の説明
